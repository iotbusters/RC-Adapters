...
void CRSF::begin(SoftwareSerial *port, unsigned long baud)
{
    m_port = port;
...
void CRSF::readPacket()
{
    ...
    while (m_port->available() > 0)
    {
        ...
        if (bufferIndex == 0)
        {
            ...
            {
                m_inBuffer[bufferIndex++] = inData; // [0]: address
                inData = m_port->read();
                frameLength = inData;
                m_inBuffer[bufferIndex++] = inData; // [1]: frame length
            }
            else
            {
                bufferIndex = 0;
        ...
        {
            m_inBuffer[bufferIndex++] = inData; // [2..frame length]: frame type + frame payload
        }
        else if (bufferIndex == frameLength + 1)
        {
            // calculate received packet crc
            m_inBuffer[bufferIndex++] = inData; // [frame length + 1]: CRC from TX
            auto crc = crsf_crc8(&m_inBuffer[2], frameLength - 1);
            ...
            if (frameLength == CRSF_FRAME_LENGTH && m_inBuffer[0] == CRSF_ADDRESS_FLIGHT_CONTROLLER)
            {
                if (crc == inData)
                {
                    ...
                    memset(m_inBuffer, 0, CRSF_PACKET_SIZE);
