...
void Es900Rx::begin() { this->crossfire.begin(&this->crossfireSerial, 115200); }
...
const bool Es900Rx::tryRead() {
    ...
    if (this->linked != this->crossfire.isConnected()) {
        ...
        println(this->linked ? "ES900RX LINKED" : "ES900RX UNLINKED");
    }

    if (this->armed != this->isArmed()) {
        this->armed = !this->armed;
        println(this->armed ? "ES900RX ARMED" : "ES900RX DISARMED");
    }

    return this->linked;
}

RxOutput Es900Rx::getOutput() const { return this->linked ? RxOutput(this->getSteering(), this->getThrottle()) : RxOutput::unlinked; }
...
bool Es900Rx::isArmed() const { return this->crossfire.getChannel(5) > ES900RX_AUX_MIN; }
